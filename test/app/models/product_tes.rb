...
describe "Product Model" do 
  ...
  should "create a new product model instance" do        
    product = Category.new(:title => Faker::Name.name, :desc => Faker::Lorem.paragraphs(2).join(" "), :price => 5005)   
    product.save
    assert product.errors.size == 0
  end                
  
  should "add an image to a product" do        
    product       = Category.new(:title => Faker::Name.name, :desc => Faker::Lorem.paragraphs(2).join(" "), :price => 5005)   
    product.image = File.open(File.expand_path(File.dirname(__FILE__) + '/../../data/ProductImage.jpg'))
    product.save
    assert product.errors.size == 0
  end
